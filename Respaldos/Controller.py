...
# TABLA cosechavigente
print("Inicio logica para determinar cosecha vigente")
i = 0
bdg_cursor.execute("select ID, tipovino, cosecha, "
                   "max(ifnull(cubas,0)+ifnull(embotellado,0)+ifnull(barrica,0)-"
                   "ifnull(pedidos,0)-ifnull(reserva,0)) as disponibilidad "
                   "from stagekupay.vista_enbodega where cosecha >= 18 and cosecha <= 22 and "
                   "length(cosecha)>0 "
                   "group by ID, tipovino, cosecha order by tipovino asc, cosecha asc")
print("tabla cosechavigente")
for ID, tipovino, cosecha, disponibilidad in bdg_cursor.fetchall():
    i = i + 1
    if not disponibilidad == 0:
        sql = "insert into stagekupay.cosechavigente(id, tipo_vino, cosecha, disponibilidad, marca) " \
              "VALUES (%s, %s, %s, (%s / 9), 0)"
...
# TABLA cosechavigente marcar vigente para venta
i = 0
bdg_cursor.execute(
    "select ID, tipo_vino, cosecha, disponibilidad, marca from stagekupay.cosechavigente "
    "order by tipo_vino asc, cosecha asc")
print("Marcar cosecha vigente")
for ID, tipo_vino, cosecha, disponibilidad, marca in bdg_cursor.fetchall():
    i = i + 1
    if disponibilidad > varLimite:
...
print("Logica para determinar disponible")
# Sumando Barricas
i = 0
...
for tipovino, factorano in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set tab_vinos.Año = RIGHT(((1000+(tab_vinos.cosecha*1)) + " \
          "IFNULL(%s,0)),2) where tipovino = %s"
...
bdg_cursor.execute(
    "select right(ano,2) as ano , sum(cajas9lts) as cajas9L, Tipo_vino as tipovino "
    ...
    "group by right(ano,2), Tipo_vino")
for ano, cajas9L, tipovino in bdg_cursor.fetchall():
    i = i + 1
...
# Agregando Ppto2023
i = 0
print(" Agregando ppto 2023")
bdg_cursor.execute(
    "select (sum(cajas9lts)) as cajas9L, Tipo_vino as tipovino from stagekupay.presupuesto2023 "
    "join stagekupay.vista_producto on presupuesto2023.codproducto = vista_producto.codproducto "
    "group by Tipo_vino")
for cajas9L, tipovino in bdg_cursor.fetchall():
    i = i + 1
    cajas9L = cajas9L
    # varPorcCrecimiento
    sql = "update stagekupay.tab_vinos set tab_vinos.venta = %s where tipovino = %s and año = '23'"
...
bdg_cursor.execute(
    "select ID, tipovino, cosecha, (select sum(ifnull(venta,0)) from stagekupay.tab_vinos as t "
    "where t.tipovino = tab_vinos.tipovino and t.cosecha=tab_vinos.cosecha+1) as venta, "
    "(select sum(disponible) from stagekupay.tab_vinos as t "
    "where t.tipovino = tab_vinos.tipovino and t.cosecha=tab_vinos.cosecha-1) "
    "as disponible, (select sum(necprod) from stagekupay.tab_vinos as t "
    "where t.tipovino = tab_vinos.tipovino and t.cosecha=tab_vinos.cosecha-1) as necesidad "
    "from stagekupay.tab_vinos where vigente='1'")
for ID, tipovino, cosecha, venta, disponible, necesidad in bdg_cursor.fetchall():
    i = i + 1
...
bdg_cursor.execute(
    "select ID, tipovino, cosecha, ifnull(disponible,0), ifnull(ventaAnt,ifnull(venta,0)) as ventaAnt, "
...
i = 0
bdg_cursor.execute("SELECT Tipo_vino, sum(Cajas9Lts) as Cajas9Lts FROM stagekupay.vista_ventaactual_agrupado "
                   "join stagekupay.vista_producto "
                   "on (vista_producto.CodProducto = vista_ventaactual_agrupado.codproducto) "
                   "where length(Tipo_vino)>0 group by Tipo_vino")
for Tipo_vino, Cajas9Lts in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set VentaCaja9Lts = %s  where año = '23' and tipovino = %s "
...
i = 0
bdg_cursor.execute(
    "SELECT Tipo_vino, sum(C9Lts) as Cajas9Lts FROM stagekupay.vista_panelactual left "
    "join stagekupay.vista_producto on (vista_producto.CodProducto = vista_panelactual.codproducto) "
    "where length(Tipo_vino)>0 group by Tipo_vino")
for Tipovino, Cajas9Lts in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set PanelCaja9Lts = %s  where año = '23' and tipovino = %s "
...
i = 0
bdg_cursor.execute(
    "SELECT Tipo_vino, sum(Cajas9Lts) as Cajas9Lts FROM stagekupay.presupuesto2023 "
    " join stagekupay.vista_producto on (vista_producto.CodProducto = presupuesto2023.codproducto) "
    "where length(Tipo_vino)>0 group by Tipo_vino;")
for Tipovino, Cajas9Lts in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set PptoCaja9Lts = %s  where año = '23' and tipovino = %s "
    val = (Cajas9Lts, Tipovino)
    bdg_cursor.execute(sql, val)
    bdg.commit()
...
# Sumatoria de disponibilidad de Saldos
i = 0
print("Sumatoria de disponibilidad de Saldos")
bdg_cursor.execute("select tipo_vino, disponibilidad, ID from stagekupay.cosechavigente where ID in "
                   "(select ID from stagekupay.tab_vinos where vigente=1)")
for tipovino, disponible, ID in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set tab_vinos.dispReal = %s, " \
          "tab_vinos.semaforo = %s  where tipovino = %s and ID = %s"
...
# Agregandosemaforo
i = 0
print(" Agregando venta año anterior ")
bdg_cursor.execute("select sum(ifnull(cajas9lts,0)) as cajas9L, Tipo_vino as tipovino "
                   "from stagekupay.vista_vta_historica_comp "
                   "join stagekupay.vista_producto on vista_vta_historica_comp.codproducto = vista_producto.codproducto"
                   " where ano = '2021' group by right(ano,2), Tipo_vino")
for cajas9L, tipovino in bdg_cursor.fetchall():
    i = i + 1
    sql = "update stagekupay.tab_vinos set tab_vinos.semaforo = (tab_vinos.semaforo/%s)*12 " \
          "where tipovino = %s and año = '22'"
    val = (cajas9L, tipovino)
    bdg_cursor.execute(sql, val)
    bdg.commit()
