...
if kupay_cursor.rowcount <= 0:
    ...
    bdg_cursor.execute("TRUNCATE TABLE " + EsquemaBD + ".bdg_modfichaexpo")
    ...
    # TABLA bdg_pedido 46
    i = 0
    kupay_cursor.execute("select Ficha, Quien, Que, ValorAntes, ValorDespues, Fecha, Comentario, "
                         "Hora, CodProducto, NumPedido, NumOrden from modfichaexpo")
    registrosorigen = kupay_cursor.rowcount
    print("(46) tabla bdg_modfichaexpo")
    for Ficha, Quien, Que, ValorAntes, ValorDespues, Fecha, Comentario, Hora, CodProducto, \
        NumPedido, NumOrden in kupay_cursor.fetchall():
        i = i + 1
        sql = "INSERT INTO " + EsquemaBD + ".bdg_modfichaexpo (Ficha, Quien, Que, ValorAntes, ValorDespues, " \
                                           "Fecha, Comentario, Hora, CodProducto, NumPedido, NumOrden) " \
                                           "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        val = (Ficha, Quien, Que, ValorAntes, ValorDespues, Fecha, Comentario, Hora, CodProducto, NumPedido, NumOrden)
    ...
    val = (Identificador, SistemaOrigen, 'modfichaexpo', 'bdg_modfichaexpo', registrosorigen, i, fechacarga)
    bdg_cursor.execute(sql, val)
    bdg.commit()
