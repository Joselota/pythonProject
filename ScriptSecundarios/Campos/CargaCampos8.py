...
def main():
    ...
    bdg_cursor.execute("DELETE FROM " + EsquemaBD + ".LaboresPer2023 where year(fecha)=2024")
    print("Fin del proceso de truncado de tablas en " + EsquemaBD + " ")
    ...
    # Tabla FROM `LaboresPer`
    i = 0
    campos_cursor.execute('SELECT CodMovDia, CodLabor, CodCuartel, Tiempo, Valor, Trato, Bono, Costo, ValorDiarioMes, '
                          'CodPer, Fecha, CodContratista, CodFaena, Unidad, CodTipoPlanta, IDLiqContratista, CodHilera, '
                          'CodVariedad, PagaTrato, HorasExtra, IDLaboresPer, CodBono, QHE, FechaIngreso, NumCtoCtta, '
                          'MontoPorSemCorr, Folio, UsuarioIng FROM LaboresPer where year(fecha)=2024')
    registrosorigen = campos_cursor.rowcount
    print("(103) tabla LaboresPer2023")
    ...
    for CodMovDia, CodLabor, CodCuartel, Tiempo, Valor, Trato, Bono, Costo, ValorDiarioMes, CodPer, Fecha, CodContratista, CodFaena, Unidad, CodTipoPlanta, IDLiqContratista, CodHilera, CodVariedad, PagaTrato, HorasExtra, IDLaboresPer, CodBono, QHE, FechaIngreso, NumCtoCtta, MontoPorSemCorr, Folio, UsuarioIng in campos_cursor.fetchall():
        ...
        sql = "INSERT INTO " + EsquemaBD + ".LaboresPer2023 (CodMovDia, CodLabor, CodCuartel, Tiempo, Valor, Trato, Bono, " \
                                           "Costo, ValorDiarioMes, CodPer, Fecha, CodContratista, CodFaena, Unidad, " \
                                           "CodTipoPlanta, IDLiqContratista, CodHilera, CodVariedad, PagaTrato, " \
                                           "HorasExtra, IDLaboresPer, CodBono, QHE, FechaIngreso, NumCtoCtta, " \
                                           "MontoPorSemCorr, Folio, UsuarioIng) " \
    ...
    val = (Identificador, SistemaOrigen, 'LaboresPer2023', 'LaboresPer2023', registrosorigen, i, fechacarga)
    bdg_cursor.execute(sql, val)
    bdg.commit()
    ...
    envio_mail("Fin proceso de Cargar Tablas en Campos 8/9")
