...
#SELECT `TcCodigo`, `TcDescribe`, `TcPagoDia`, `TcConHaberes`, `TcConDctos`, `TcConFeriados`, `TcConSemCorr`, `TcConAFP`, `TcConIsapre`, `TcCodRem`, `TcTipoPlazo`, `TcPagoMes`, `TcPagoFaena`, `TcSegCesantiaDctoTrab` FROM `TipoContratos`
i = 0
campos_cursor.execute('SELECT `TcCodigo`, `TcDescribe`, `TcPagoDia`, `TcConHaberes`, `TcConDctos`, `TcConFeriados`, `TcConSemCorr`, `TcConAFP`, `TcConIsapre`, `TcCodRem`, `TcTipoPlazo`, `TcPagoMes`, `TcPagoFaena`, `TcSegCesantiaDctoTrab` FROM TipoContratos')
registrosorigen = campos_cursor.rowcount
print("(1) tabla TipoContratos")
print(registrosorigen)
for TcCodigo, TcDescribe, TcPagoDia, TcConHaberes, TcConDctos, TcConFeriados, TcConSemCorr, TcConAFP, TcConIsapre, TcCodRem, TcTipoPlazo, TcPagoMes, TcPagoFaena, TcSegCesantiaDctoTrab in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".TipoContratos (TcCodigo, TcDescribe, TcPagoDia, TcConHaberes, TcConDctos, TcConFeriados, TcConSemCorr, TcConAFP, TcConIsapre, TcCodRem, TcTipoPlazo, TcPagoMes, TcPagoFaena, TcSegCesantiaDctoTrab) " \
...
#SELECT `Temporada`, `FechaInicio`, `FechaTermino`, `Campo`, `Observaciones`, `CodEmpresa` FROM `Temporadas`
i = 0
campos_cursor.execute('SELECT `Temporada`, `FechaInicio`, `FechaTermino`, `Campo`, `Observaciones`, `CodEmpresa` FROM Temporadas')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Temporadas")
print(registrosorigen)
for Temporada, FechaInicio, FechaTermino, Campo, Observaciones, CodEmpresa in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Temporadas (Temporada, FechaInicio, FechaTermino, Campo, Observaciones, CodEmpresa) " \
                                       "VALUES (%s, %s, %s, %s, %s, %s)"
...
#SELECT `NumSol`, `Fecha`, `Entrega`, `Nota`, `Neto`, `Iva`, `Total`, `Estado`, `Solicita`, `Aprueba`, `CodPrioridad`, `ODC`, `CodCC`, `CodProv`, `NomProv`, `Lugar`, `Atencion`, `NomSolicita`, `Exenta`, `ComentEstado`, `NomAprueba`, `Alternativas`, `Recibe`, `NomRecibe`, `SinTraspaso`, `FechaAprb`, `Desde_Campos`, `BdgCodigo`, `Digita`, `NomDigita`, `NumSIC` FROM `SolicitudODC`
i = 0
campos_cursor.execute('SELECT `NumSol`, `Fecha`, `Entrega`, `Nota`, `Neto`, `Iva`, `Total`, `Estado`, `Solicita`, `Aprueba`, `CodPrioridad`, `ODC`, `CodCC`, `CodProv`, `NomProv`, `Lugar`, `Atencion`, `NomSolicita`, `Exenta`, `ComentEstado`, `NomAprueba`, `Alternativas`, `Recibe`, `NomRecibe`, `SinTraspaso`, `FechaAprb`, `Desde_Campos`, `BdgCodigo`, `Digita`, `NomDigita`, `NumSIC` FROM SolicitudODC')
registrosorigen = campos_cursor.rowcount
print("(1) tabla SolicitudODC")
print(registrosorigen)
for NumSol, Fecha, Entrega, Nota, Neto, Iva, Total, Estado, Solicita, Aprueba, CodPrioridad, ODC, CodCC, CodProv, NomProv, Lugar, Atencion, NomSolicita, Exenta, ComentEstado, NomAprueba, Alternativas, Recibe, NomRecibe, SinTraspaso, FechaAprb, Desde_Campos, BdgCodigo, Digita, NomDigita, NumSIC in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".SolicitudODC (NumSol, Fecha, Entrega, Nota, Neto, Iva, Total, Estado, Solicita, Aprueba, CodPrioridad, ODC, CodCC, CodProv, NomProv, Lugar, Atencion, NomSolicita, Exenta, ComentEstado, NomAprueba, Alternativas, Recibe, NomRecibe, SinTraspaso, FechaAprb, Desde_Campos, BdgCodigo, Digita, NomDigita, NumSIC) " \
...
#SELECT `CodCom`, `Fecha`, `CodServ`, `Cantidad`, `Valor`, `ValorPesos`, `Sub_Total`, `NomSer`, `NumODC`, `CodCC`, `Hora`, `BdgCodigo`, `CodCtaCtble`, `NumDocAso`, `TipoDocAso`, `Tipo`, `Mes`, `IvaRec`, `IvaNoRec`, `NLineaODC`, `IDDetRecep`, `CantidadNC` FROM `RecepcionServicios`
i = 0
campos_cursor.execute('SELECT `CodCom`, `Fecha`, `CodServ`, `Cantidad`, `Valor`, `ValorPesos`, `Sub_Total`, `NomSer`, `NumODC`, `CodCC`, `Hora`, `BdgCodigo`, `CodCtaCtble`, `NumDocAso`, `TipoDocAso`, `Tipo`, `Mes`, `IvaRec`, `IvaNoRec`, `NLineaODC`, `IDDetRecep`, `CantidadNC` FROM RecepcionServicios')
registrosorigen = campos_cursor.rowcount
print("(1) tabla RecepcionServicios")
print(registrosorigen)
for CodCom, Fecha, CodServ, Cantidad, Valor, ValorPesos, Sub_Total, NomSer, NumODC, CodCC, Hora, BdgCodigo, CodCtaCtble, NumDocAso, TipoDocAso, Tipo, Mes, IvaRec, IvaNoRec, NLineaODC, IDDetRecep, CantidadNC in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".RecepcionServicios (CodCom, Fecha, CodServ, Cantidad, Valor, ValorPesos, Sub_Total, NomSer, NumODC, CodCC, Hora, BdgCodigo, CodCtaCtble, NumDocAso, TipoDocAso, Tipo, Mes, IvaRec, IvaNoRec, NLineaODC, IDDetRecep, CantidadNC) " \
...
#SELECT `CodProv`, `NomProv`, `Direccion`, `Giro`, `Fono`, `Fax`, `Ciudad`, `Region`, `Rubro`, `Vendedor`, `EsContratista`, `EsConcesionario`, `LugarPago` FROM `Proveedores`
i = 0
campos_cursor.execute('SELECT `CodProv`, `NomProv`, `Direccion`, `Giro`, `Fono`, `Fax`, `Ciudad`, `Region`, `Rubro`, `Vendedor`, `EsContratista`, `EsConcesionario`, `LugarPago` FROM Proveedores')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Proveedores")
print(registrosorigen)
...
for CodPer, NomPer, DireccionPer, CodCargo, CodAfp, CodIsa, NumCargas, MinimoImponible, Select, Cuadrilla, ValDiaPer, FechaNacPer, EstadoCivilPer, Contratado, TipoContrato, FechaInicio, FechaTermino, Disponible, SueldoFijo, CiudadPer, Finiquitado, CodLabor, UnicoFundo, Calificacion, MotivoFiniquito, CalcSemCorr, CalcFeriados, ValDiaMinimo, ValDiaPer31, ValDiaPer28, TotDetalleFiniquito, TextDesgloseFiniquito, ConCesantia, FonoContacto, CesantiaCargoEmp, PlanIsapreUF, FechaIngreso, Liquido_Bruto, PagoTratosImponible, Nombres, ApellidoPaterno, ApellidoMaterno, Sexo, LugarNacimiento, Nacionalidad, eMail, CodBanco, CodCuentaBanco, CategoriaRem700, EsContratista, CodContratista, Comentarios, AlimentacionEjecutiva, LugarIntegro, FpCodigo, TcCodigo, CodCajaComp, CodMutual, Bloqueado, PlanIsaprePesos, AvisoFiniquito, CodClasifica, LugarPagoRem700, JornadaRem700, Profesion, Escolaridad, CalJuridicaRem700, Administrativo, DescribeFaena, FolioVigente, RangoCarga, FechaFirmaFiniquito, Renuncia, RentaFIja, FiniquitoConLiquida, ValDiaPer29, SinGratificacion, DiasSemanaPer, PagaSabado, ComoPagaSabado, HoraInicioAM, HoraTerminoPM, HoraInicioAMSab, HoraTerminoPMSab, Jubilado, CodAFC, CodAPV, PesosAPV, UFAPV, PorcenAPV, InicioAPV, TerminoAPV, PesosAV, UFAV, InicioAV, TerminoAV, PesosCVA, UFCVA, InicioCVA, TerminoCVA, CodAV, CodCVA, Poliza, HoraTerminoAM, HoraInicioPM, HoraTerminoAMSab, HoraInicioPMSab, PorcAV, PorcCVA, polizaAV, PolizaCVA, GrupoC, Division, SeguroC_11agnos in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Personal (CodPer, NomPer, DireccionPer, CodCargo, CodAfp, CodIsa, NumCargas, MinimoImponible, Seleccion, Cuadrilla, ValDiaPer, FechaNacPer, EstadoCivilPer, Contratado, TipoContrato, FechaInicio, FechaTermino, Disponible, SueldoFijo, CiudadPer, Finiquitado, CodLabor, UnicoFundo, Calificacion, MotivoFiniquito, CalcSemCorr, CalcFeriados, ValDiaMinimo, ValDiaPer31, ValDiaPer28, TotDetalleFiniquito, TextDesgloseFiniquito, ConCesantia, FonoContacto, CesantiaCargoEmp, PlanIsapreUF, FechaIngreso, Liquido_Bruto, xContrato, xFiniquito, PagoTratosImponible, Nombres, ApellidoPaterno, ApellidoMaterno, Sexo, LugarNacimiento, Nacionalidad, eMail, CodBanco, CodCuentaBanco, CategoriaRem700, EsContratista, CodContratista, Comentarios, AlimentacionEjecutiva, LugarIntegro, FpCodigo, TcCodigo, CodCajaComp, CodMutual, Bloqueado, PlanIsaprePesos, AvisoFiniquito, CodClasifica, LugarPagoRem700, JornadaRem700, Profesion, Escolaridad, CalJuridicaRem700, Administrativo, DescribeFaena, FolioVigente, RangoCarga, FechaFirmaFiniquito, Renuncia, RentaFIja, FiniquitoConLiquida, ValDiaPer29, SinGratificacion, DiasSemanaPer, PagaSabado, ComoPagaSabado, HoraInicioAM, HoraTerminoPM, HoraInicioAMSab, HoraTerminoPMSab, Jubilado, CodAFC, CodAPV, PesosAPV, UFAPV, PorcenAPV, InicioAPV, TerminoAPV, PesosAV, UFAV, InicioAV, TerminoAV, PesosCVA, UFCVA, InicioCVA, TerminoCVA, CodAV, CodCVA, Poliza, HoraTerminoAM, HoraInicioPM, HoraTerminoAMSab, HoraInicioPMSab, PorcAV, PorcCVA, polizaAV, PolizaCVA, GrupoC, Division, SeguroC_11agnos) " \
                                       "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
for Gratificacion, TopeImponible, CodHEx1, CodHEx2, CodFamSer, CodFamRep, FactHEx1M, FactHEx2M, TopeCesantia, FormLicenciasR700, CodLabTrato, CiudadEmp, DomicilioEmp, RazonSocialEmp, CodSemCorr, RUTEmp, CodFeriado, CodVacacion, ValorIngresoMinimo, ValorIVA, PrecisionSueldos, AporteIndefinido, AporteFijo, TopeGratificacion, CodColacion, CodMovilizacion, TieneRem700, CodSabado, CodDomingo, OpeContabFin700, CodLicencia, CodCalMermaProd, CtaFacturasPagar, CtaIvaCredito, CtaOtrosImpuestos, CodMoneda, BodegaPpal, CorrConsumos, PorcRetencion, CtaRetencion, CtaHonoraPagar, PathServer, CorrLiqODC, IPServerKupay, NombreMaquina, ConConexionKupay, DiasDeTrabajo, ClaveDesignerKupay, FonoEmp, FaxEmp, GiroEmp, RutRepLegal, PaisEmp, xLista, CorrelativoBaseDatos, Host, ToeMail, EliminaDespliegeValores, EliminaModStockFactura, ODBCDataSource, ODBCName, ODBCPass, ODBCDataSourceF700, ODBCNameF700, ODBCPassF700, PorUnaFin700, DivFin700, UniFin700, IdTesoreriaFin700, IdContabFin700, IdLCoFin700, IdLVeFin700, CtaIESNoRecuperable, CtaIESRecuperable, ProcedimientosGlobal, LeyendaOC, PagoPorHilera, PorcAnticipo, MaximoHE, CodBonoSobreTpo, LeyendaFacturarA, LeyendaAnexa, CodEmp, ImpMembreteDocCpra, MembreteODC, TDFacturaCpra, TDNotaCredito, TDNotaDebito, ValorHEContratista, TDIngreso, TDEgreso, TDTraspaso, TDGuiaCompra, TDAplicacion, CodBonoTratos, PagoTratos, FormAusenciasR700, PathODC, CalculoCargasFicha, ConPreAprobacion, DiasVacaciones, TipoFiniquito, TarjaMesB30, IPServerCampos, NomMaquinaCampos, ClaveDesignerCampos, PathBPA, TopeHExMes, HEx2SinMaximos, FactHEx1D, FactHEx2D, BonoMensual, FactHEx1D2, MesInv, AgnoInv, VersionFIN700, ODBCDsnR700, ODBCNombreR700, ODBCClaveR700, CodEmpR700, TDGuiaDevolucion, CtaFacturasRecibir, TDBoletaHonorario, CtaNotaPorRecibir, CtaResultado, TituloLiquidacion, FactCpraServMaqConDist, TDAjusteInventario, ToleranciaDocum, LabCostoEmpR700, LabFiniquitoR700, FormDiasLiqR700, FormSemCorrR700, TipoProyectoR700, DiasBaseBonoTratos, VersionREM700, GeneraArchivos, RemArchivoPlano, ValidaCuartelAreaDpto in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Parametros (Gratificacion, TopeImponible, CodHEx1, CodHEx2, CodFamSer, CodFamRep, FactHEx1M, FactHEx2M, TopeCesantia, FormLicenciasR700, CodLabTrato, CiudadEmp, DomicilioEmp, RazonSocialEmp, CodSemCorr, RUTEmp, CodFeriado, CodVacacion, ValorIngresoMinimo, ValorIVA, PrecisionSueldos, AporteIndefinido, AporteFijo, TopeGratificacion, CodColacion, CodMovilizacion, TieneRem700, CodSabado, CodDomingo, OpeContabFin700, CodLicencia, CodCalMermaProd, CtaFacturasPagar, CtaIvaCredito, CtaOtrosImpuestos, CodMoneda, BodegaPpal, CorrConsumos, PorcRetencion, CtaRetencion, CtaHonoraPagar, PathServer, CorrLiqODC, IPServerKupay, NombreMaquina, ConConexionKupay, DiasDeTrabajo, ClaveDesignerKupay, FonoEmp, FaxEmp, GiroEmp, RutRepLegal, PaisEmp, xLista, CorrelativoBaseDatos, Host, ToeMail, EliminaDespliegeValores, EliminaModStockFactura, ODBCDataSource, ODBCName, ODBCPass, ODBCDataSourceF700, ODBCNameF700, ODBCPassF700, PorUnaFin700, DivFin700, UniFin700, IdTesoreriaFin700, IdContabFin700, IdLCoFin700, IdLVeFin700, CtaIESNoRecuperable, CtaIESRecuperable, ProcedimientosGlobal, LeyendaOC, PagoPorHilera, PorcAnticipo, MaximoHE, CodBonoSobreTpo, LeyendaFacturarA, LeyendaAnexa, CodEmp, ImpMembreteDocCpra, MembreteODC, TDFacturaCpra, TDNotaCredito, TDNotaDebito, ValorHEContratista, TDIngreso, TDEgreso, TDTraspaso, TDGuiaCompra, TDAplicacion, CodBonoTratos, PagoTratos, FormAusenciasR700, PathODC, CalculoCargasFicha, ConPreAprobacion, DiasVacaciones, TipoFiniquito, TarjaMesB30, IPServerCampos, NomMaquinaCampos, ClaveDesignerCampos, PathBPA, TopeHExMes, HEx2SinMaximos, FactHEx1D, FactHEx2D, BonoMensual, FactHEx1D2, MesInv, AgnoInv, VersionFIN700, ODBCDsnR700, ODBCNombreR700, ODBCClaveR700, CodEmpR700, TDGuiaDevolucion, CtaFacturasRecibir, TDBoletaHonorario, CtaNotaPorRecibir, CtaResultado, TituloLiquidacion, FactCpraServMaqConDist, TDAjusteInventario, ToleranciaDocum, LabCostoEmpR700, LabFiniquitoR700, FormDiasLiqR700, FormSemCorrR700, TipoProyectoR700, DiasBaseBonoTratos, VersionREM700, GeneraArchivos, RemArchivoPlano, ValidaCuartelAreaDpto) " \
                                       "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
#SELECT `NumFolioFC`, `FechaInicio`, `FechaTermino`, `NomPeriodo`, `CodPeriodo`, `DiasPeriodo`, `MesPeriodo`, `AgnoPeriodo`, `DiasHabilesPer` FROM `Periodo`
i = 0
campos_cursor.execute('SELECT `NumFolioFC`, `FechaInicio`, `FechaTermino`, `NomPeriodo`, `CodPeriodo`, `DiasPeriodo`, `MesPeriodo`, `AgnoPeriodo`, `DiasHabilesPer` FROM Periodo')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Periodo")
print(registrosorigen)
for NumFolioFC, FechaInicio, FechaTermino, NomPeriodo, CodPeriodo, DiasPeriodo, MesPeriodo, AgnoPeriodo, DiasHabilesPer in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Periodo (NumFolioFC, FechaInicio, FechaTermino, NomPeriodo, CodPeriodo, DiasPeriodo, MesPeriodo, AgnoPeriodo, DiasHabilesPer) " \
...
#SELECT `CodCtaCtble`, `TipoCtaCtble`, `NomCtaCtble`, `xAtributos` FROM `PlanCuentas`
i = 0
campos_cursor.execute('SELECT `CodCtaCtble`, `TipoCtaCtble`, `NomCtaCtble`, `xAtributos` FROM PlanCuentas')
registrosorigen = campos_cursor.rowcount
print("(1) tabla PlanCuentas")
print(registrosorigen)
...
#SELECT `Agno`, `CodCuartel`, `HasCuartel`, `ValorHa`, `IC_Unico`, `CantidadPpto`, `ValorUnitPpto`, `TotalCantidad`, `TotalValor`, `CodigoPpto`, `Mes`, `DescPpto`, `RealCantidad`, `RealValor`, `ParaBorrar` FROM `PptoDetalleCC`
i = 0
campos_cursor.execute('SELECT `Agno`, `CodCuartel`, `HasCuartel`, `ValorHa`, `IC_Unico`, `CantidadPpto`, `ValorUnitPpto`, `TotalCantidad`, `TotalValor`, `CodigoPpto`, `Mes`, `DescPpto`, `RealCantidad`, `RealValor`, `ParaBorrar` FROM PptoDetalleCC')
registrosorigen = campos_cursor.rowcount
print("(1) tabla PptoDetalleCC")
print(registrosorigen)
for Agno, CodCuartel, HasCuartel, ValorHa, IC_Unico, CantidadPpto, ValorUnitPpto, TotalCantidad, TotalValor, CodigoPpto, Mes, DescPpto, RealCantidad, RealValor, ParaBorrar in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".PptoDetalleCC (Agno, CodCuartel, HasCuartel, ValorHa, IC_Unico, CantidadPpto, ValorUnitPpto, TotalCantidad, TotalValor, CodigoPpto, Mes, DescPpto, RealCantidad, RealValor, ParaBorrar) " \
...
#SELECT `Tipo`, `Estado`, `CodItem`, `DescItem`, `Cantidad`, `ValorUnitario`, `IC_Unico`, `CodCultivo` FROM `PptoItems`
i = 0
campos_cursor.execute('SELECT `Tipo`, `Estado`, `CodItem`, `DescItem`, `Cantidad`, `ValorUnitario`, `IC_Unico`, `CodCultivo` FROM PptoItems')
registrosorigen = campos_cursor.rowcount
print("(1) tabla PptoItems")
print(registrosorigen)
for Tipo, Estado, CodItem, DescItem, Cantidad, ValorUnitario, IC_Unico, CodCultivo in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".PptoItems (Tipo, Estado, CodItem, DescItem, Cantidad, ValorUnitario, IC_Unico, CodCultivo) " \
...
#SELECT `IC_Unico`, `Codigo`, `Cantidad`, `ValorUnitario`, `Descripcion` FROM `PptoItemsDetalle`
i = 0
campos_cursor.execute('SELECT `IC_Unico`, `Codigo`, `Cantidad`, `ValorUnitario`, `Descripcion` FROM PptoItemsDetalle')
registrosorigen = campos_cursor.rowcount
print("(1) tabla PptoItemsDetalle")
print(registrosorigen)
...
#SELECT `CodPrioridad`, `NomPrioridad`, `Dias` FROM `Prioridad`
i = 0
campos_cursor.execute('SELECT `CodPrioridad`, `NomPrioridad`, `Dias` FROM Prioridad')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Prioridad")
print(registrosorigen)
...
#SELECT `PrdCod`, `PrdFecha`, `PrdAgno`, `PrdTipo`, `PrdCorrelativo`, `Supervisor`, `OperadorGrua`, `TrasladoCosecha`, `Despachadora`, `Anotador`, `NumBins`, `Tractorista` FROM `Produccion`
i = 0
campos_cursor.execute('SELECT `PrdCod`, `PrdFecha`, `PrdAgno`, `PrdTipo`, `PrdCorrelativo`, `Supervisor`, `OperadorGrua`, `TrasladoCosecha`, `Despachadora`, `Anotador`, `NumBins`, `Tractorista` FROM Produccion')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Produccion")
print(registrosorigen)
for PrdCod, PrdFecha, PrdAgno, PrdTipo, PrdCorrelativo, Supervisor, OperadorGrua, TrasladoCosecha, Despachadora, Anotador, NumBins, Tractorista in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Produccion (PrdCod, PrdFecha, PrdAgno, PrdTipo, PrdCorrelativo, Supervisor, OperadorGrua, TrasladoCosecha, Despachadora, Anotador, NumBins, Tractorista) " \
...
#SELECT `CodProd`, `CodFam`, `CodSubFam`, `Unidad`, `Costo`, `NomProd`, `Stock`, `StockMinimo`, `Vencimiento`, `AplicaEnf`, `Caracteristicas`, `CorrecMonetaria`, `CodCtaCtble`, `CostoPMP`, `ManejaVencimiento`, `ProductoNacional`, `CostoFinal`, `CuentaGastos`, `Tolerancia`, `TotalExistenciaVal`, `Carencia`, `Retorno`, `StockCorrec`, `FechaUltCpra`, `TotalExisValo`, `TotalBdg`, `StockBdg` FROM `Productos`
i = 0
campos_cursor.execute('SELECT `CodProd`, `CodFam`, `CodSubFam`, `Unidad`, `Costo`, `NomProd`, `Stock`, `StockMinimo`, `Vencimiento`, `AplicaEnf`, `Caracteristicas`, `CorrecMonetaria`, `CodCtaCtble`, `CostoPMP`, `ManejaVencimiento`, `ProductoNacional`, `CostoFinal`, `CuentaGastos`, `Tolerancia`, `TotalExistenciaVal`, `Carencia`, `Retorno`, `StockCorrec`, `FechaUltCpra`, `TotalExisValo`, `TotalBdg`, `StockBdg` FROM Productos')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Productos")
print(registrosorigen)
for CodProd, CodFam, CodSubFam, Unidad, Costo, NomProd, Stock, StockMinimo, Vencimiento, AplicaEnf, Caracteristicas, CorrecMonetaria, CodCtaCtble, CostoPMP, ManejaVencimiento, ProductoNacional, CostoFinal, CuentaGastos, Tolerancia, TotalExistenciaVal, Carencia, Retorno, StockCorrec, FechaUltCpra, TotalExisValo, TotalBdg, StockBdg in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Productos (CodProd, CodFam, CodSubFam, Unidad, Costo, NomProd, Stock, StockMinimo, Vencimiento, AplicaEnf, Caracteristicas, CorrecMonetaria, CodCtaCtble, CostoPMP, ManejaVencimiento, ProductoNacional, CostoFinal, CuentaGastos, Tolerancia, TotalExistenciaVal, Carencia, Retorno, StockCorrec, FechaUltCpra, TotalExisValo, TotalBdg, StockBdg) " \
...
#SELECT `CodCarga`, `RangoImponible`, `ValorCarga`, `CodRango` FROM `RangoCargas`
i = 0
campos_cursor.execute('SELECT `CodCarga`, `RangoImponible`, `ValorCarga`, `CodRango` FROM RangoCargas')
registrosorigen = campos_cursor.rowcount
print("(1) tabla RangoCargas")
print(registrosorigen)
...
#SELECT `CodSer`, `NomSer`, `CodFam`, `CodSubFam`, `Costo`, `Tipo`, `CodCtaCtble` FROM `Servicios`
i = 0
campos_cursor.execute('SELECT `CodSer`, `NomSer`, `CodFam`, `CodSubFam`, `Costo`, `Tipo`, `CodCtaCtble` FROM Servicios')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Servicios")
print(registrosorigen)
...
#SELECT `CodSubFam`, `NomSubFam`, `CodFam`, `UnicoSubFam`, `CodCtaCtble` FROM `SubFamilias`
i = 0
campos_cursor.execute('SELECT `CodSubFam`, `NomSubFam`, `CodFam`, `UnicoSubFam`, `CodCtaCtble` FROM SubFamilias')
registrosorigen = campos_cursor.rowcount
print("(1) tabla SubFamilias")
print(registrosorigen)
...
#SELECT `CodCarga`, `NomCarga` FROM `TipoCargas`
i = 0
campos_cursor.execute('SELECT `CodCarga`, `NomCarga` FROM TipoCargas')
registrosorigen = campos_cursor.rowcount
print("(1) tabla TipoCargas")
print(registrosorigen)
...
#SELECT `TipoDoc`, `NomDoc`, `CuentaDoc`, `IdFin700`, `OpeFin700` FROM `TipoDocumentos`
i = 0
campos_cursor.execute('SELECT `TipoDoc`, `NomDoc`, `CuentaDoc`, `IdFin700`, `OpeFin700` FROM TipoDocumentos')
registrosorigen = campos_cursor.rowcount
print("(1) tabla TipoDocumentos")
print(registrosorigen)
...
#SELECT `CodTipoPlanta`, `NomTipoPlanta` FROM `TipoPlantas`
i = 0
campos_cursor.execute('SELECT `CodTipoPlanta`, `NomTipoPlanta` FROM TipoPlantas')
registrosorigen = campos_cursor.rowcount
print("(1) tabla TipoPlantas")
print(registrosorigen)
...
#SELECT `DesdeUTM`, `Factor`, `RebajaUTM`, `UnicoUTM`, `DesdePesos`, `RebajaPesos` FROM `TramosImpuesto`
i = 0
campos_cursor.execute('SELECT `DesdeUTM`, `Factor`, `RebajaUTM`, `UnicoUTM`, `DesdePesos`, `RebajaPesos` FROM TramosImpuesto')
registrosorigen = campos_cursor.rowcount
print("(1) tabla TramosImpuesto")
print(registrosorigen)
for DesdeUTM, Factor, RebajaUTM, UnicoUTM, DesdePesos, RebajaPesos in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".TramosImpuesto (DesdeUTM, Factor, RebajaUTM, UnicoUTM, DesdePesos, RebajaPesos) " \
                                       "VALUES (%s, %s, %s, %s, %s, %s)"
...
#SELECT `CodigoTP`, `Valor`, `FechaVigencia` FROM `ValoresTiposParametros`
i = 0
campos_cursor.execute('SELECT CodigoTP, Valor, FechaVigencia FROM ValoresTiposParametros')
registrosorigen = campos_cursor.rowcount
print("(1) tabla ValoresTiposParametros")
print(registrosorigen)
...
#SELECT `UFDia`, `UFValor` FROM `ValoresUF`
i = 0
campos_cursor.execute('SELECT UFDia, UFValor FROM ValoresUF')
registrosorigen = campos_cursor.rowcount
print("(1) tabla ValoresUF")
print(registrosorigen)
...
#SELECT `MesUTM`, `AgnoUTM`, `ValorUTM`, `UnicoUTM` FROM `ValoresUTM`
i = 0
campos_cursor.execute('SELECT MesUTM, AgnoUTM, ValorUTM, UnicoUTM FROM ValoresUTM')
registrosorigen = campos_cursor.rowcount
print("(1) tabla ValoresUTM")
print(registrosorigen)
...
#SELECT `CodVariedad`, `NomVariedad`, `CodCultivo`, `TotProd`, `TotMaqui`, `TotMMOO`, `TotCtta`, `TotGastos`, `TotFactDir`, `TotCmpte`, `TotCosto`, `TotIngre`, `UtilVarie`, `KilosCosecha`, `CostoTotalKilos`, `CostoKilo`, `IngresoKilos`, `PromedioKilos`, `UtilidadKilo`, `TotMMOOInd`, `Has` FROM `Variedades`
i = 0
campos_cursor.execute('SELECT CodVariedad, NomVariedad, CodCultivo, TotProd, TotMaqui, TotMMOO, TotCtta, TotGastos, TotFactDir, TotCmpte, TotCosto, TotIngre, UtilVarie, KilosCosecha, CostoTotalKilos, CostoKilo, IngresoKilos, PromedioKilos, UtilidadKilo, TotMMOOInd, Has FROM Variedades')
registrosorigen = campos_cursor.rowcount
print("(1) tabla Variedades")
print(registrosorigen)
for CodVariedad, NomVariedad, CodCultivo, TotProd, TotMaqui, TotMMOO, TotCtta, TotGastos, TotFactDir, TotCmpte, TotCosto, TotIngre, UtilVarie, KilosCosecha, CostoTotalKilos, CostoKilo, IngresoKilos, PromedioKilos, UtilidadKilo, TotMMOOInd, Has in campos_cursor.fetchall():
    ...
    sql = "INSERT INTO " + EsquemaBD + ".Variedades (CodVariedad, NomVariedad, CodCultivo, TotProd, TotMaqui, TotMMOO, TotCtta, TotGastos, TotFactDir, TotCmpte, TotCosto, TotIngre, UtilVarie, KilosCosecha, CostoTotalKilos, CostoKilo, IngresoKilos, PromedioKilos, UtilidadKilo, TotMMOOInd, Has) " \
